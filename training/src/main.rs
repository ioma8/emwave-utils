...
fn reader_loop(shared: Arc<Mutex<Snapshot>>) {
    loop {
        match emwave::Device::open_and_start() {
            Ok(dev) => {
                    ...
                    match dev.read_report(150) {
                        Ok(Some(rep)) => {
                            if rep[0] == 0x75 {
                                ...
                                let snap = stream.snapshot("connected", true);
                                *shared.lock() = snap;
                            }
                        }
                        Ok(None) => {}
        ...
        std::thread::sleep(Duration::from_secs(1)); // retry
    }
}
...
fn main() -> eframe::Result {
    ...
    std::thread::spawn(move || reader_loop(reader_shared));
